...
namespace Latihan_Pos
{
    public partial class Modify_Staff : Form
    {
        ...
        void reset()
        {
            command = new MySqlCommand("select ifnull(max(id),0)+1 from Staff", koneksi);
        ...
        void showAll()
        {
            command = new MySqlCommand("select * from Staff", koneksi);
            MyD_Adapter = new MySqlDataAdapter(command);
            MyD_Table = new DataTable();
            MyD_Adapter.Fill(MyD_Table);
        ...
        void filteredSelect()
        {
            command = new MySqlCommand("select * from Staff where _code like concat('%', @_code, '%') and _name like concat('%', @_name, '%')", koneksi);
        ...
        void insertData()
        {
            ...
            {
                koneksi.Open();
                command = new MySqlCommand("insert into Staff values(@id,@_code,@_name,@_Address,@Born,@Mutated)", koneksi);
        ...
        void updateData()
        {
            ...
            try
            {
                command = new MySqlCommand("update Staff set _code=@_code,_name=@_name,_Address=@_Address,Born=@Born,Mutated=@Mutated where id=@id", koneksi);
                command.Parameters.AddWithValue("@id", TextBox_ID.Text);
                command.Parameters.AddWithValue("@_code", TextBox_Kode.Text);
                command.Parameters.AddWithValue("@_name", TextBox_Nama.Text);
                command.Parameters.AddWithValue("@_Address", TextBox_Alamat.Text);
