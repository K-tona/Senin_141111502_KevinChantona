...
{
    public partial class Modify_Barang : Form
    {
        string max;
        ...
        void reset()
        {
            command = new MySqlCommand("select ifnull(max(id),0)+1 from Things", koneksi);
            ...
            TextBox_Nama.Text = "";
            TextBox_Jumlah.Text = "";
            TextBox_HargaHPP.Text = "";
            TextBox_HargaJual.Text = "";
        }
        ...
        void filteredSelect()
        {
            command = new MySqlCommand("select * from Things where _code like concat('%', @_code, '%') and _name like concat('%', @_name, '%')", koneksi);
        ...
        void insertData()
        {
            ...
            {
                koneksi.Open();
                command = new MySqlCommand("insert into Things values(@id,@_code,@_name,@_FirstSUM,@_Hppprice,@_SalePrice,@Born,@Mutated)", koneksi);
                command.Parameters.AddWithValue("@id", TextBox_ID.Text);
                command.Parameters.AddWithValue("@_code", TextBox_Kode.Text);
                command.Parameters.AddWithValue("@_name", TextBox_Nama.Text);
                command.Parameters.AddWithValue("@_FirstSUM", int.Parse(TextBox_Jumlah.Text));
                command.Parameters.AddWithValue("@_Hppprice", Convert.ToDecimal(TextBox_HargaHPP.Text));
                command.Parameters.AddWithValue("@_SalePrice", Convert.ToDecimal(TextBox_HargaJual.Text));
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.SelectedCells.Count > 0)
            {
                ...
                TextBox_Jumlah.Text = dataGridView1.Rows[index].Cells[3].Value.ToString();
                TextBox_HargaHPP.Text = dataGridView1.Rows[index].Cells[4].Value.ToString();
                TextBox_HargaJual.Text = dataGridView1.Rows[index].Cells[5].Value.ToString();
